...
if __name__ == "__main__":
    ...
    # 篩選條件
    para_keys = {'region', 'searchtype', 'section', 'mrtline', 'mrtcoods', 'school', 'busness',
                 'kind', 'multiPrice', 'rentprice', 'multiRoom', 'other', 'showMore', 'shape',
                 'multiArea', 'area', 'multiFloor', 'option', 'multiNotice'}
    ...
    sort_params = {
        # 租金由小到大
        'order': 'posttime',  # posttime, area, money
        'orderType': 'desc'  # asc
    }
    ...
    for key in para_keys:
        pattern = f"{key}=(.*?)(?:&|$)"
        match = re.search(pattern, web)
        if match:
            filter_params[key] = match.group(1)
    ...
    i = 0
    while 1:
        house_detail = house591_spider.get_house_detail(houses[i]['post_id'])
        ...
        price = house_detail['favData']['price']
        address = house_detail['positionRound']['address']
        floor = house_detail['info'][2]['value']
        kind = house_detail['favData']['kindTxt']
        posttime = house_detail['publish']['postTime']

        data = [title, price ,address ,floor, kind, posttime, url]

        #search the data posted in 12 hours
        if translate_to_minutes(posttime) > 12*60:
            break;
